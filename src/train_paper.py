...
def train():
    ...
    for epoch in range(n_epochs):
        ...
        for i in range(0, len(x_data) - seq_len, batch_size):
            ...
            # 🔥 Fix: Create input and target tensors for Transformer decoder
            x_input = x_batch[:, :-1]  # Everything except the last token
            x_target = x_batch[:, 1:]  # Everything except the first token

            # 🔥 Fix: Use ignore_index=-100 for masked loss computation
            padding_mask = (x_target == 0).to(device)  # Mask positions with padding
